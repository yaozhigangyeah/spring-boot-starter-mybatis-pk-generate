package com.pk.generater.interceptor;

import com.pk.generater.annotation.Id;
...
public class GeneratePkInterceptor implements Interceptor {

    private static final String ID_TYPE = "idType";

    private final PkGenerateFactory pkGenerateFactory;
    ...
    public GeneratePkInterceptor(PkGenerateFactory pkGenerateFactory) {
        this.pkGenerateFactory = pkGenerateFactory;
    }
    ...
    private void generatorKey(List<Field> fieldList, Object paramObj, String idType) throws Exception {
        if (fieldList.size() > 0) {
            for (Field field : fieldList) {
                pkGenerateFactory.doHandler(field.getAnnotation(Id.class).value().getKey(), paramObj, field);
            }
        } else { // 无主键注解时执行
            pkGenerateFactory.doHandler(idType, paramObj, null);
        }
    }
}